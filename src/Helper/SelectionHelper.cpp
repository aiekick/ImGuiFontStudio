...
static bool __clickedFromLastFrame = false;
...
void SelectionHelper::DrawMenu(ProjectFile *vProjectFile)
{
	if (!m_SelectionForOperation.empty())
	{
		if (ImGui::BeginFramedGroup("Selection Operations"))
		{
			...
			ImGui::Text("Re Range Code Points :");

			ImGui::Indent();
			{
				ImGui::Text("Max Range : 0 > 65535");
				if (!m_ReRangeStruct.startCodePoint.valid)
					ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.8f, 0.2f, 0.2f, 0.8f));
				ImGui::PushItemWidth(100.0f);
				bool edited = ImGui::InputInt("Start CodePoint", &m_ReRangeStruct.startCodePoint.codePoint);
				ImGui::PopItemWidth();
				if (!m_ReRangeStruct.startCodePoint.valid)
					ImGui::PopStyleColor();
				if (edited)
				{
					...
					{
						m_ReRangeStruct.startCodePoint.valid = false;
					}
					else
					{
						m_ReRangeStruct.startCodePoint.valid = true;
					}
				}
				...
				if (!m_ReRangeStruct.endCodePoint.valid)
					ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.8f, 0.2f, 0.2f, 0.8f));
				ImGui::PushItemWidth(100.0f);
				edited = ImGui::InputInt("End CodePoint", &m_ReRangeStruct.endCodePoint.codePoint);
				ImGui::PopItemWidth();
				if (!m_ReRangeStruct.endCodePoint.valid)
					ImGui::PopStyleColor();
				if (edited)
				{
					...
					{
						m_ReRangeStruct.endCodePoint.valid = false;
					}
					else
					{
						m_ReRangeStruct.endCodePoint.valid = true;
					}
				}
	...
	{
		ImGui::Text("Selection Type by Mouse : ");
		...
		ImGui::Indent();
		{
			ImGui::RadioButtonLabeled_BitWize<GlyphSelectionTypeFlags>("Zone", "by Zone",
				&m_GlyphSelectionTypeFlags, GlyphSelectionTypeFlags::GLYPH_SELECTION_TYPE_BY_ZONE, 50.0f, true);
			ImGui::SameLine();
			ImGui::RadioButtonLabeled_BitWize<GlyphSelectionTypeFlags>("Range", "by Range",
				&m_GlyphSelectionTypeFlags, GlyphSelectionTypeFlags::GLYPH_SELECTION_TYPE_BY_RANGE, 50.0f, true);
			ImGui::SameLine();
			ImGui::RadioButtonLabeled_BitWize<GlyphSelectionTypeFlags>("Line", "by Line",
				&m_GlyphSelectionTypeFlags, GlyphSelectionTypeFlags::GLYPH_SELECTION_TYPE_BY_LINE, 50.0f, true);

			if (IsSelectionType(GlyphSelectionTypeFlags::GLYPH_SELECTION_TYPE_BY_ZONE))
			{
				ImGui::Indent();
				{
					ImGui::Text("Zone Radius :");
					ImGui::Indent();
					{
						if (ImGui::Button("R##Radius"))
							m_Zone.z = m_Zone.w;
						ImGui::SameLine();
						ImGui::PushItemWidth(60.0f);
						ImGui::SliderFloat("##radius", &m_Zone.z, 0.5f, 150.0f);
						ImGui::PopItemWidth();
					}
					ImGui::Unindent();
				}
		...
		if (!IsSelectionType(GlyphSelectionTypeFlags::GLYPH_SELECTION_TYPE_BY_RANGE))
		{
			...
			ImGui::Indent();
			{
				ImGui::RadioButtonLabeled_BitWize<GlyphSelectionModeFlags>(
					...
					&m_GlyphSelectionModeFlags, GlyphSelectionModeFlags::GLYPH_SELECTION_MODE_ADD, 50.0f, true);
				ImGui::SameLine();
				ImGui::RadioButtonLabeled_BitWize<GlyphSelectionModeFlags>(
					"Inv", "Select glyphs in inverse mode",
					&m_GlyphSelectionModeFlags, GlyphSelectionModeFlags::GLYPH_SELECTION_MODE_INVERSE, 50.0f, true);
			}
			ImGui::Unindent();
		}
...
bool SelectionHelper::IsGlyphIntersectedAndSelected(
	FontInfos *vFontInfos, ImVec2 vCellSize, ImWchar vCodePoint, bool *vSelected,
	SelectionContainerEnum vSelectionContainerEnum)
{
...
void SelectionHelper::UnSelectGlyph(ProjectFile *vProjectFile, FontInfos *vFontInfos, ImFontGlyph vGlyph, bool vUpdateMaps,
	SelectionContainerEnum vSelectionContainerEnum)
{
...
void SelectionHelper::ReRange_Offset_After_Start(ProjectFile *vProjectFile, ImWchar vOffsetCodePoint)
{
...
void SelectionHelper::ReRange_Offset_Before_End(ProjectFile *vProjectFile, ImWchar vOffsetCodePoint)
{
	std::set<ImWchar> codePoints;
	for (auto &font : vProjectFile->m_Fonts)
	{
		...
		if (vFontInfos)
		{
			...
			else if (vSelectionContainerEnum == SelectionContainerEnum::SELECTION_CONTAINER_FINAL)
			{
				if (vFontInfos->m_SelectedGlyphs.find(vGlyph.Codepoint) != vFontInfos->m_SelectedGlyphs.end())
				{
					...
					ImWchar leftCodePoint = vGlyph.Codepoint;
					ImWchar rightCodePoint = vGlyph.Codepoint;
					while (leftIdx != glyphs->end() || rightIdx != glyphs->end())
					{
						if (leftIdx != glyphs->end())
						{
...
void SelectionHelper::AnalyseSourceSelection(ProjectFile *vProjectFile)
{
	if (vProjectFile && vProjectFile->IsLoaded())
	{
		...
		for (auto &font : vProjectFile->m_Fonts)
		{
			...
			//	Generator::Instance()->ProhibitStatus(GeneratorStatusFlags::GENERATOR_STATUS_FONT_BATCH_ALLOWED);
			}
			else
			{
			//	Generator::Instance()->AllowStatus(GeneratorStatusFlags::GENERATOR_STATUS_FONT_BATCH_ALLOWED);
			}
...
void SelectionHelper::FinalizeSelectionForOperations()
{
	...
	ImWchar inf = 65535, sup = 0;
	for (auto & it : m_SelectionForOperation)
	{
		inf = ct::mini<ImWchar>(inf, it.first);
		sup = ct::maxi<ImWchar>(sup, it.first);
	}
