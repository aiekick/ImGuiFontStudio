...
{
    ImGui_ImplVulkanH_DestroyWindow(g_Instance, g_Device, &MainFrame::sMainWindowData, g_Allocator);
}
...
int main(int, char** argv)
{
    ...
    GLFWwindow* mainWindow = glfwCreateWindow(1280, 720, "ImGuiFontStudio", NULL, NULL);
    if (mainWindow == 0)
        return 1;
    // Setup Vulkan
    ...
    VkResult err = glfwCreateWindowSurface(g_Instance, mainWindow, g_Allocator, &surface);
    check_vk_result(err);
    ...
    MainFrame::sVulkanInitInfo = {};
    MainFrame::sVulkanInitInfo.Instance = g_Instance;
    MainFrame::sVulkanInitInfo.PhysicalDevice = g_PhysicalDevice;
    MainFrame::sVulkanInitInfo.Device = g_Device;
    MainFrame::sVulkanInitInfo.QueueFamily = g_QueueFamily;
    MainFrame::sVulkanInitInfo.Queue = g_Queue;
    MainFrame::sVulkanInitInfo.PipelineCache = g_PipelineCache;
    MainFrame::sVulkanInitInfo.DescriptorPool = g_DescriptorPool;
    MainFrame::sVulkanInitInfo.Allocator = g_Allocator;
    MainFrame::sVulkanInitInfo.MinImageCount = g_MinImageCount;
    MainFrame::sVulkanInitInfo.ImageCount = wd->ImageCount;
    MainFrame::sVulkanInitInfo.CheckVkResultFn = check_vk_result;
    ImGui_ImplVulkan_Init(&MainFrame::sVulkanInitInfo, wd->RenderPass);
    ...
    while (!glfwWindowShouldClose(mainWindow))
    {
        ...
        if (g_SwapChainRebuild)
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, &MainFrame::sMainWindowData, g_QueueFamily, g_Allocator, width, height, g_MinImageCount);
                MainFrame::sMainWindowData.FrameIndex = 0;
